...
QUERY_PROPOSAL_DELIVERY = QUERY_PROPOSAL_SHIPPING # Minimized for refactor readability - typically the full string applies
...
class ShopifyCheckoutSession:
    ...
    async def fetch_product_if_needed(self, session) -> Tuple[bool, str]:
        if not self.variant_id:
            if self.site_url in _VARIANT_CACHE:
                ...
                try:
                    async with session.get(f"{self.site_url}/products.json?limit=250", proxy=self.proxy, timeout=30) as resp:
                        if resp.status != 200: return False, f"Site Error! Status: {resp.status}"
                        data = await resp.json()
                        products = data.get('products', [])
                        if not products: return False, "No Products Found"
                        
                        # Simplified logic to grab a cheap variant
                        min_product = None
                        min_price = float('inf')
                            ...
                            for variant in product.get('variants', []):
                                ...
                                try:
                                    ...
                                    if 0 < price < min_price:
                                        min_price = price
                                        min_product = str(variant['id'])
                                except: pass
                        if not min_product:
                            return False, "No Valid Products"
                        self.variant_id = min_product
                        _VARIANT_CACHE[self.site_url] = self.variant_id
                except Exception as e:
                    return False, f"Proxy/Fetch Error: {str(e)}"
        return True, "OK"
    ...
    async def negotiate_shipping(self, session) -> Tuple[bool, str]:
        json_data = {
            ...
            'variables': {
                ...
                'payment': {'totalAmount': {'any': True}, 'paymentLines': [], 'billingAddress': {'streetAddress': {'countryCode': self.country_code}}},
                'buyerIdentity': {
                    ...
                    'marketingConsent': [{'email': {'value': self.email}}], 'shopPayOptInPhone': {'countryCode': self.country_code}
                },
                'tip': {'tipLines': []},
                'taxes': {'proposedTotalAmount': {'value': {'amount': '0', 'currencyCode': self.currency}}, 'proposedExemptions': []},
                'note': {'customAttributes': []}, 'localizationExtension': {'fields': []}
            },
        ...
        try:
            ...
            if 'errors' in resp:
                return False, f"GraphQL Error: {resp['errors'][0].get('message', 'Unknown')}"
            
            result = resp['data']['session']['negotiate']['result']
            rtype = result.get('__typename')
            if rtype in ('CheckpointDenied', 'Throttled', 'NegotiationResultFailed'):
                return False, rtype
                
            self.checkpoint_data = result.get('checkpointData')
            sp = result.get('sellerProposal', {})
            
            self.running_total = sp.get('runningTotal', {}).get('value', {}).get('amount', self.subtotal)
            
            # Extract Delivery
            delivery = sp.get('delivery', {})
            if delivery.get('__typename') == 'FilledDeliveryTerms':
                dl = delivery.get('deliveryLines', [{}])[0]
                strats = dl.get('availableDeliveryStrategies', [])
                if strats:
                    self.delivery_strategy = strats[0].get('handle', '')
                    self.shipping_amount = float(strats[0].get('amount', {}).get('value', {}).get('amount', '0'))
            
            # Extract Tax
            tax = sp.get('tax', {})
            if tax.get('__typename') == 'FilledTaxTerms':
                self.tax_amount = float(tax.get('totalTaxAmount', {}).get('value', {}).get('amount', '0'))
                
            # Extract Payment Method
            payment = sp.get('payment', {})
            if payment.get('__typename') == 'FilledPaymentTerms':
                for pm in payment.get('availablePaymentLines', []):
                    method = pm.get('paymentMethod', {})
                    if method.get('paymentMethodIdentifier'):
                        self.payment_identifier = method.get('paymentMethodIdentifier')
                        self.gateway = method.get('extensibilityDisplayName') or method.get('name', 'UNKNOWN')
                        break
                        
            if not self.payment_identifier:
                return False, "No valid payment method found"
                
            return True, "OK"
        except Exception as e:
            return False, f"Failed parse proposal: {str(e)}"
        ...
        json_data = {
            ...
            'variables': {
                ...
                'merchandise': {
                    'merchandiseLines': [{
                        'stableId': self.stableId,
                        'merchandise': {'productVariantReference': {'id': f'gid://shopify/ProductVariantMerchandise/{self.merch}', 'variantId': f'gid://shopify/ProductVariant/{self.variant_id}'}},
                        'quantity': {'items': {'value': 1}},
                        'expectedTotalPrice': {'value': {'amount': self.subtotal, 'currencyCode': self.currency}},
                        'lineComponents': []
                    }]
                ...
                'payment': {
                    ...
                    'billingAddress': {'streetAddress': {
                        'address1': self.street, 'city': self.city, 'countryCode': self.country_code, 'postalCode': self.s_zip,
                ...
                'tip': {'tipLines': []},
                'taxes': {'proposedTotalAmount': {'value': {'amount': str(self.tax_amount), 'currencyCode': self.currency}}, 'proposedExemptions': []},
                'note': {'customAttributes': []}, 'localizationExtension': {'fields': []}
            },
            'operationName': 'Proposal'
        }
        return await self._graphql_req(session, json_data)
    ...
    async def submit_payment(self, session, token: str) -> Tuple[bool, str, str]:
            ...
            'input': {
                ...
                'delivery': {
                    'deliveryLines': [{
                        'destination': {
                            'streetAddress': {
                                'address1': self.street, 'city': self.city, 'countryCode': self.country_code,
                                'postalCode': self.s_zip, 'firstName': self.firstName, 'lastName': self.lastName,
                                'zoneCode': self.state, 'phone': self.phone
                            }
                        },
                        'selectedDeliveryStrategy': {'deliveryStrategyByHandle': {'handle': self.delivery_strategy, 'customDeliveryRate': False}, 'options': {'phone': self.phone}},
                        'targetMerchandiseLines': {'lines': [{'stableId': self.stableId}]},
                        'deliveryMethodTypes': ['SHIPPING'],
                        'expectedTotalPrice': {'value': {'amount': str(self.shipping_amount), 'currencyCode': self.currency}},
                        'destinationChanged': False
                    }],
                    'useProgressiveRates': True, 'supportsSplitShipping': True
                },
                'merchandise': {
                    'merchandiseLines': [{
                        'stableId': self.stableId,
                ...
                'payment': {
                    ...
                    'billingAddress': {'streetAddress': {'address1': self.street, 'city': self.city, 'countryCode': self.country_code, 'postalCode': self.s_zip, 'firstName': self.firstName, 'lastName': self.lastName, 'zoneCode': self.state, 'phone': self.phone}}
                },
                'buyerIdentity': {
                    'customer': {'presentmentCurrency': self.currency, 'countryCode': self.country_code},
                    'email': self.email, 'phoneCountryCode': self.country_code, 'shopPayOptInPhone': {'number': self.phone, 'countryCode': self.country_code}
                },
                'taxes': {'proposedTotalAmount': {'value': {'amount': str(self.tax_amount), 'currencyCode': self.currency}}, 'proposedExemptions': []},
                'tip': {'tipLines': []}, 'note': {'customAttributes': []}
            },
        ...
        try:
            ...
            if not sd: return False, "Empty submit response", ""
...
def shopify_checker():
    try:
        ...
        try:
            total_val = float(price) if price else 0.0
            ...
            price_str = f"${price} [Min Prod: ${subtotal} | Tax: ${tax} | Ship: ${ship}]"
