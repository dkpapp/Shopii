...
async def find_cheapest_product(site, proxy_str=None):
    res = await fetch_products(site, proxy_str)
    if isinstance(res, tuple) and res[0] is False:
        return None, None, None, res[1]
    link = res.get('link', '')
    product_handle = link.split('/')[-1] if '/' in link else ''
    try:
        price = float(res.get('price', 0))
    except Exception:
        price = 0.0
    return res.get('variant_id'), price, product_handle, 'OK'
