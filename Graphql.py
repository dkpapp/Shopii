...
# we permanently bypass dynamic schema deprecation errors (orderingIndex, IdealBank, Redressions, etc.)
# without needing a string sanitizer.
...
# The delivery proposal is structurally identical to shipping proposal
...
mutation SubmitForCompletion($input: NegotiationInput!, $attemptToken: String!) {
  submitForCompletion(input: $input, attemptToken: $attemptToken) {
    __typename
    ... on SubmitSuccess { receipt { id __typename } }
    ... on SubmitAlreadyAccepted { receipt { id __typename } }
    ... on SubmittedForCompletion { receipt { id __typename } }
