...
ADDRESS_BOOK = {
    ...
    "CA": {"address1": "88 Queen St", "city": "Toronto", "postalCode": "M5J 2J3", "zoneCode": "ON", "countryCode": "CA", "phone": "4165550198"},
    "GB": {"address1": "221B Baker St", "city": "London", "postalCode": "NW1 6XE", "zoneCode": "LND", "countryCode": "GB", "phone": "2079460123"},
    "AU": {"address1": "1 Martin Pl", "city": "Sydney", "postalCode": "2000", "zoneCode": "NSW", "countryCode": "AU", "phone": "0291234567"},
    "FR": {"address1": "15 Rue de la Paix", "city": "Paris", "postalCode": "75002", "zoneCode": "IDF", "countryCode": "FR", "phone": "0142685300"},
    "IN": {"address1": "221B MG Rd", "city": "Mumbai", "postalCode": "400001", "zoneCode": "MH", "countryCode": "IN", "phone": "+919876543210"},
...
class ShopifyCheckoutSession:
    ...
    def _match_address_to_currency(self) -> None:
        """Dynamically shifts the shipping address to the store's domestic country to minimize shipping costs."""
        curr_map = {
            'USD': 'US', 'CAD': 'CA', 'GBP': 'GB', 'AUD': 'AU',
            'EUR': 'FR', 'INR': 'IN', 'AED': 'AE'
        }
        cc = curr_map.get(self.currency.upper(), 'US')
        if cc in ADDRESS_BOOK:
            self.address_info = ADDRESS_BOOK[cc]
        else:
            self.address_info = ADDRESS_BOOK['US']
                    ...
                    for product in products:
                        for variant in product.get('variants', []):
                            ...
                            try:
                                ...
                                # Strictly find the absolute minimum priced product across all endpoints
                                if price < min_price:
                                    min_price = price
    ...
    async def init_checkout(self, session) -> Tuple[bool, str]:
        ...
        async with session.post(self.site_url + '/checkout/', allow_redirects=True, headers=checkout_headers, proxy=self.proxy) as resp:
            ...
            # Match shipping address locally to the parsed backend currency
            self._match_address_to_currency()
    ...
    async def negotiate_shipping(self, session) -> Tuple[bool, str]:
        ...
        for attempt in range(3):
            ...
            try:
                ...
                # Shipping Amount & Strategy (Pick the cheapest available)
...
async def process_card_async(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None) -> Tuple[bool, str, str, str, str, str, float, float]:
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            if not ok:
                # Cache invalidation fallback for stale/out-of-stock variants
                if checkout.site_url in _VARIANT_CACHE:
                    ...
                    if ok:
                        ok, msg = await checkout.add_to_cart(session)
                if not ok:
                    return False, msg, checkout.gateway, checkout.running_total, checkout.currency, checkout.subtotal, checkout.tax_amount, checkout.shipping_amount
