...
class ShopifyCheckoutSession:
    ...
    async def poll_receipt(self, session, receipt_id: str) -> Tuple[bool, str]:
        ...
        # Strict fallback using JSON-like structures instead of loose text searches
        code = extract_between(last_text, '{"code":"', '"')
        if code: return True, code
        
        stripped_text = last_text.replace(' ', '')
        if '"__typename":"ProcessedReceipt"' in stripped_text: return True, "ORDER_PLACED"
        if '"__typename":"ActionRequiredReceipt"' in stripped_text: return True, "OTP_REQUIRED"
