...
class ShopifyCheckoutSession:
    ...
    def get_navigate_headers(self) -> Dict[str, str]:
        """Headers used strictly for standard browser page navigation to avoid GraphQL mismatches."""
        return {
    ...
    def get_graphql_headers(self) -> Dict[str, str]:
        """Headers tuned perfectly for Shopify's backend JSON GraphQL API endpoint."""
        h = {
            'accept': '*/*',
    ...
    async def fetch_product_if_needed(self, session) -> Tuple[bool, str]:
        ...
        for endpoint in endpoints:
            try:
                resp = await session.get(endpoint, headers=headers, timeout=15)
