...
# Optimized Global Address Book for minimizing State/Provincial Sales tax & minimizing regional logic errors
# --- DYNAMIC SCHEMA SANITIZER ---
def sanitize_graphql_schema(q_str: str) -> str:
    """Removes deprecated schema fields that cause Shopify to reject the query."""
    replacements = [
        # Fixes: "Field 'orderingIndex' doesn't exist on type 'PaymentOnDeliveryMethod'"
        # Fixes: "Field 'name' doesn't exist on type 'PaymentOnDeliveryMethod'"
        # Fixes: "Field 'availablePresentmentCurrencies' doesn't exist on type 'PaymentOnDeliveryMethod'"
        ("orderingIndex name availablePresentmentCurrencies", ""),
        ("orderingIndex availablePresentmentCurrencies", ""),
        ("orderingIndex displayName", "displayName"),
        ("orderingIndex paypalAccountEmail", "paypalAccountEmail"),
        ("orderingIndex clientToken", "clientToken"),
        ("orderingIndex showRedirectionNotice", "showRedirectionNotice"),
        (" paymentMethodIdentifier orderingIndex", " paymentMethodIdentifier"),
        
        # Fixes: "Fragment on IdealBankSelectionParameterConfig can't be spread"
...
QUERY_PROPOSAL_DELIVERY = sanitize_graphql_schema(QUERY_PROPOSAL_DELIVERY)
MUTATION_SUBMIT = sanitize_graphql_schema(MUTATION_SUBMIT)
QUERY_POLL = sanitize_graphql_schema(QUERY_POLL)


# Optimized Global Address Book for minimizing State/Provincial Sales tax & minimizing regional logic errors
ADDRESS_BOOK = {
    # United States (Delaware - 0% State Sales Tax)
    "US": {
        "address1": "1201 N Market St", "address2": "Suite 100", "city": "Wilmington",
        "zoneCode": "DE", "postalCode": "19801", "countryCode": "US", "phone": "+13025550143"
    },
    # Canada (Alberta - Lowest 5% GST, No Provincial Sales Tax)
    "CA": {
        "address1": "700 2 St SW", "address2": "", "city": "Calgary",
        "zoneCode": "AB", "postalCode": "T2P 2W2", "countryCode": "CA", "phone": "+14035550187"
    },
    # United Kingdom (Central London Domestic Hub)
    "GB": {
        "address1": "100 New Bridge St", "address2": "", "city": "London",
        "zoneCode": "LND", "postalCode": "EC4V 6JA", "countryCode": "GB", "phone": "+442079460912"
    },
    # Australia (Sydney Metro)
    "AU": {
        "address1": "100 George St", "address2": "", "city": "Sydney",
        "zoneCode": "NSW", "postalCode": "2000", "countryCode": "AU", "phone": "+61291234567"
    },
    # Germany / EU Central (Berlin Hub)
    "DE": {
        "address1": "Friedrichstraße 43", "address2": "", "city": "Berlin",
        "zoneCode": "BE", "postalCode": "10117", "countryCode": "DE", "phone": "+493012345678"
    },
    # France (Paris Metro)
    "FR": {
        "address1": "8 Boulevard de la Madeleine", "address2": "", "city": "Paris",
        "zoneCode": "IDF", "postalCode": "75009", "countryCode": "FR", "phone": "+33142685300"
    },
    # India (Mumbai Commercial Center)
    "IN": {
        "address1": "Bandra Kurla Complex, G Block", "address2": "", "city": "Mumbai",
        "zoneCode": "MH", "postalCode": "400051", "countryCode": "IN", "phone": "+919820012345"
    },
    # United Arab Emirates (Dubai Commercial Zone)
    "AE": {
        "address1": "Sheikh Zayed Rd, Trade Centre 1", "address2": "", "city": "Dubai",
        "zoneCode": "DU", "postalCode": "00000", "countryCode": "AE", "phone": "+97143321111"
    },
    # Default Fallback (US Tax-Free)
    "DEFAULT": {
        "address1": "1201 N Market St", "address2": "Suite 100", "city": "Wilmington",
        "zoneCode": "DE", "postalCode": "19801", "countryCode": "US", "phone": "+13025550143"
    }
...
def resolve_address(url: str, currency: Optional[str] = None) -> Dict[str, str]:
    """Resolves the domestic shipping profile based on currency and domain TLD."""
    domain = urlparse(url if url.startswith("http") else f"https://{url}").netloc.lower()
    tld = domain.split(".")[-1].upper()
    
    # Priority 1: Check if TLD maps directly to an address
    if tld in ADDRESS_BOOK:
        return ADDRESS_BOOK[tld]
        
    # Priority 2: Match by actual checkout currency
    if currency:
        country_code = CURRENCY_TO_COUNTRY.get(currency.upper())
        if country_code and country_code in ADDRESS_BOOK:
            return ADDRESS_BOOK[country_code]
...
def get_curl_proxy(proxy_str: str) -> Optional[Dict[str, str]]:
    """Converts a standard proxy string to the dict format expected by curl_cffi."""
...
class ShopifyCheckoutSession:
    def __init__(self, site_url: str, cc_info: Dict[str, str], variant_id: Optional[str] = None, proxy_str: Optional[str] = None):
        # Force HTTPS to prevent insecure redirect loops that trigger Cloudflare CAPTCHAs
        if site_url.startswith('http://'):
            self.site_url = site_url.replace('http://', 'https://')
        ...
        # Initial address resolution (will be updated after we know the currency)
        ...
        # Upgraded human-like headers to bypass Cloudflare and Datadome initial checks
        self.headers = {
    ...
    async def _graphql_req(self, session, json_data, max_retries=1) -> Tuple[bool, str]:
        ...
        for attempt in range(max_retries + 1):
            try:
                resp = await session.post(graphql_url, params=params, headers=self.headers, json=json_data)
                resp_text = resp.text
    ...
    def _sync_address_to_currency(self) -> None:
        """Dynamically shifts the address to match the backend currency to avoid international rates."""
    ...
    async def fetch_product_if_needed(self, session) -> Tuple[bool, str]:
        ...
        for endpoint in endpoints:
            try:
                resp = await session.get(endpoint, timeout=20)
    ...
    async def add_to_cart(self, session) -> Tuple[bool, str]:
        ...
        try:
            resp = await session.post(cart, data=f'id={self.variant_id}&quantity=1', headers=cart_headers)
            if resp.status_code != 200:
                cart_headers['Content-Type'] = 'application/json'
                r2 = await session.post(cart, json={'items': [{'id': int(self.variant_id), 'quantity': 1}]}, headers=cart_headers)
                if r2.status_code != 200: return False, f"Cart failed {r2.status_code}"
            return True, "OK"
        except Exception as e:
            return False, f"Add to cart error: {str(e)}"

    async def init_checkout(self, session) -> Tuple[bool, str]:
        # DO NOT use POST here. Browsers exclusively use GET to transition from Cart to Checkout.
        # POST requests flag Cloudflare and DataDome instantly, resulting in CAPTCHA_REQUIRED.
        checkout_headers = self.headers.copy()
        checkout_headers['Accept'] = 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8'
        
        try:
            resp = await session.get(self.site_url + '/checkout', allow_redirects=True, headers=checkout_headers)
            self.checkout_url = str(resp.url)
            text = resp.text
        except Exception as e:
            return False, f"Init checkout failed: {str(e)}"

        # 1. Immediate Captcha / Block Verification
        if is_captcha_required(text) or resp.status_code in (403, 429):
            return False, "CAPTCHA_REQUIRED (Checkout Init)"
        
        # 2. Redirect / Availability Checks
        url_lower = self.checkout_url.lower()
        if 'login' in url_lower:
            return False, "Site requires login!"
        if 'password' in url_lower:
            return False, "Site is password protected!"
        if '/cart' in url_lower and '/checkouts' not in url_lower:
            return False, "Redirected to Cart (Product OOS or Cart Empty)"

        # Improved regex to handle standard /c/, /cn/, and /unstable/ checkouts
        ...
        if not self.sst:
            # Expanded robust token extractions for newer Shopify builds
            patterns = [
                r'name="serialized-sessionToken"\s+content="([^"]+)"',
                r'"serializedSessionToken"\s*:\s*"([^"]+)"',
                r'data-session-token="([^"]+)"',
                r'"sessionToken"\s*:\s*"([^"]+)"',
                r'name="shopify-checkout-session-token"\s+content="([^"]+)"',
        ...
        if not self.sst:
            return False, f"Failed to get session token. URL: {self.checkout_url}"
        ...
        # Sync address to correct regional logic
    ...
    async def negotiate_shipping(self, session) -> Tuple[bool, str]:
            ...
            'variables': {
                ...
                'delivery': {
                    'deliveryLines': [{
                        ...
                        # Mimic native SDK: let the backend compute the cheapest default by providing wildcards
        ...
        # Extended loop to wait out PendingTerms and guarantee accurate math
        for attempt in range(6):
            ...
            try:
                ...
                if delivery.get('__typename') == 'FilledDeliveryTerms':
                    ...
                    if lines:
                        ...
                        if strats:
                            ...
                            min_price = float('inf')
                            for strat in strats:
                                amt_str = strat.get('amount', {}).get('value', {}).get('amount', '999999')
                                try:
                                    amt_float = float(amt_str)
                                    if amt_float < min_price:
                                        min_price = amt_float
                                        cheapest_strat = strat
                ...
                if not self.payment_identifier:
                    return False, "NO_DIRECT_CC_GATEWAY_AVAILABLE"
    ...
    async def negotiate_delivery(self, session) -> Tuple[bool, str]:
            ...
            'variables': {
                ...
                'delivery': {
                    'deliveryLines': [{
                        ...
                        # Lock in the specific computed handle and strict stableId linking
                        'selectedDeliveryStrategy': {
        ...
        if self.checkpoint_data: json_data['variables']['checkpointData'] = self.checkpoint_data
        
        # Increased attempts for delivery calculation confirmation
        for attempt in range(6):
            success, text = await self._graphql_req(session, json_data)
            if not success: return False, text
            ...
            try:
                resp = json.loads(text)
                if 'errors' in resp: return False, "Delivery GraphQL Error"
                    
                data = resp.get('data', {}).get('session', {}).get('negotiate', {}).get('result', {})
                if data.get('__typename') == 'NegotiationResultFailed': return False, "Delivery Negotiation Failed"
                ...
                if data:
                    ...
                    if delivery.get('__typename') == 'FilledDeliveryTerms':
                        lines = delivery.get('deliveryLines', [{}])
                        if lines:
                            # 1. First, check if the server explicitly highlighted a strategy
                            ...
                            # 2. To strictly prevent DELIVERY_DELIVERY_LINE_DETAIL_CHANGED, we must also 
                            # re-evaluate the cheapest handle in case the first handle became invalid after full address submission.
                            strats = lines[0].get('availableDeliveryStrategies', [])
                            if strats:
                                cheapest_strat = None
                                min_price = float('inf')
                                ...
                                if cheapest_strat:
                                    self.delivery_strategy = cheapest_strat.get('handle', self.delivery_strategy)
                                    amt = cheapest_strat.get('amount', {}).get('value', {}).get('amount')
                                    if amt: self.shipping_amount = str(amt)
    ...
    async def submit_payment(self, session, token: str) -> Tuple[bool, str, str]:
            ...
            'input': {
                ...
                'delivery': {
                    'deliveryLines': [{
                        ...
                        # Final strict sync to completely eliminate DELIVERY_LINE_CHANGED errors
                        'selectedDeliveryStrategy': {'deliveryStrategyByHandle': {'handle': self.delivery_strategy, 'customDeliveryRate': False}, 'options': {}},
                        'targetMerchandiseLines': {'lines': [{'stableId': self.stableId}]},
                        'deliveryMethodTypes': ['SHIPPING'], 'expectedTotalPrice': {'any': True},
                        'destinationChanged': False
                ...
                'taxes': {
                    'proposedAllocations': None, 'proposedTotalAmount': {'any': True},
                    'proposedTotalIncludedAmount': None, 'proposedMixedStateTotalAmount': None, 'proposedExemptions': []
                },
                'tip': {'tipLines': []}, 'note': {'message': None, 'customAttributes': []}, 'localizationExtension': {'fields': []},
                'nonNegotiableTerms': None, 'optionalDuties': {'buyerRefusesDuties': False}
            },
        ...
        try:
            ...
            if rtype in ('SubmitSuccess', 'SubmittedForCompletion', 'SubmitAlreadyAccepted'):
                ...
                rid = rcpt.get('id', '')
                if not rid: 
                    return False, "Missing Receipt ID in Submit", ""
    ...
    async def poll_receipt(self, session, receipt_id: str) -> Tuple[bool, str]:
        ...
        # Extended Polling to 15 iterations (up to ~30-45s) for async 3DS/Network gateways
        sleep_time = 1.5
        last_text = ""
        ...
        for _ in range(15):
            ...
            success, text = await self._graphql_req(session, json_data)
            if not success: 
                return False, text # Likely captcha trigger or connection drop
            ...
            try:
                ...
                # Check if Shopify returned a fatal GraphQL error instead of a receipt
                if 'errors' in resp:
                    err_msgs = [e.get('message', 'Unknown Error') for e in resp['errors']]
                    return False, f"Poll Error: {'; '.join(err_msgs)}"
                    
                rdata = resp.get('data', {}).get('receipt', {})
                if not rdata: 
                    break # Receipt is null or invalid, break to fallback extraction
                ...
                if tname == 'ProcessedReceipt': 
                    ...
                    # Intelligently adapt to the server's requested pollDelay (in ms) to avoid rate limits
                    delay_ms = rdata.get('pollDelay', 1500)
                    sleep_time = min(max(delay_ms / 1000.0, 1.5), 4.0)
                else:
                    break # Unrecognized receipt type, terminal state reached
                    
            except Exception: 
                break # JSON parsing failed, terminal state reached
            
        # Regex string-matching fallback in case the strict JSON extraction fails or we broke early
        if last_text:
...
async def process_card_async(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None) -> Tuple[bool, str, str, str, str, str, float, float]:
    ...
    try:
        # Using curl_cffi AsyncSession to completely spoof Chrome JA3/TLS fingerprints
        # This completely neutralizes basic Cloudflare and DataDome generic bot blocks.
        proxies = get_curl_proxy(proxy_str)
        async with AsyncSession(impersonate="chrome120", proxies=proxies, timeout=60) as session:
            
            # WARMUP: Visit the homepage first to acquire Cloudflare & Shopify _y / _s tracking cookies.
            # Crucially, this resolves custom domains (e.g. trophyburro.myshopify.com -> www.trophyburro.com),
            # preventing instant 403s and Captcha triggers when making POST requests to the wrong origin later.
            try:
                warmup_headers = checkout.headers.copy()
                warmup_headers['Accept'] = 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8'
                ...
                # Align headers dynamically to the newly resolved canonical URL
                checkout.headers['Origin'] = checkout.site_url
                checkout.headers['Referer'] = f"{checkout.site_url}/"
                
                # Fetch cart natively to generate the cart cookie securely
            ...
            if not ok:
                with _cache_lock:
                    if checkout.site_url in _VARIANT_CACHE:
                        del _VARIANT_CACHE[checkout.site_url]
        ...
        return False, f"Proxy/TLS Error: {str(e)}", checkout.gateway, checkout.running_total, checkout.currency, checkout.subtotal, checkout.tax_amount, checkout.shipping_amount
    except asyncio.TimeoutError as e:
        return False, f"Network Timeout", checkout.gateway, checkout.running_total, checkout.currency, checkout.subtotal, checkout.tax_amount, checkout.shipping_amount
    except Exception as e:
...
def shopify_checker():
    try:
        ...
        # Proper asyncio execution isolating the event loop execution
