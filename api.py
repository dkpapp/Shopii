        ...
        submit_vars = {
            'input': {
                ...
                'delivery': {
                    ...
                    'noDeliveryRequired': [{'targetMerchandiseLines': {'any': True}}] if not self.delivery_strategy else [],
                    'useProgressiveRates': True, 'prefetchShippingRatesStrategy': None, 'supportsSplitShipping': True
                },
                'merchandise': {
                    'merchandiseLines': [{
                        'stableId': self.stableId,
...
async def process_card_async(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None) -> Tuple[bool, str, str, str, str, str, float, float]:
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            # This drastically lowers the bot score and prevents instant CAPTCHA_REQUIRED triggers on strict sites.
            try:
                ...
                await session.get(checkout.site_url, headers=warmup_headers, proxy=checkout.proxy, timeout=10)
            except Exception:
                pass
