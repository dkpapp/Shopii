...
class ShopifyCheckoutSession:
        ...
        async with session.post(cart, data=f'id={self.variant_id}&quantity=1', headers=cart_headers, proxy=self.proxy) as resp:
            if resp.status != 200:
                ...
                async with session.post(cart, json={'items': [{'id': int(self.variant_id), 'quantity': 1}]}, headers=cart_headers, proxy=self.proxy) as r2:
                    if r2.status != 200: return False, f"Cart failed {r2.status}"
        return True, "OK"
    ...
    async def init_checkout(self, session) -> Tuple[bool, str]:
        ...
        async with session.get(self.site_url + '/checkout', allow_redirects=True, headers=checkout_headers, proxy=self.proxy) as resp:
            self.checkout_url = str(resp.url)
            if 'login' in self.checkout_url.lower(): return False, "Site requires login!"
            ...
            if is_captcha_required(text) or resp.status in (403, 429):
            ...
            'variables': {
                ...
                'delivery': {
                    'deliveryLines': [{
                        ...
                        # Mimic native SDK: Don't force a strategy initially, let the backend compute the cheapest default
                        'selectedDeliveryStrategy': None,
...
async def process_card_async(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None) -> Tuple[bool, str, str, str, str, str, float, float]:
    ...
    try:
        ...
        # Increased overall timeout to 60 seconds to accommodate extended polling durations
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            try:
                ...
                async with session.get(checkout.site_url, headers=warmup_headers, proxy=checkout.proxy, timeout=15, allow_redirects=True) as resp:
                ...
                await session.get(f"{checkout.site_url}/cart", headers=warmup_headers, proxy=checkout.proxy, timeout=10)
            except Exception:
                pass
