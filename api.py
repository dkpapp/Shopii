...
class ShopifyCheckoutSession:
    def __init__(self, site_url: str, cc_info: Dict[str, str], variant_id: Optional[str] = None, proxy_str: Optional[str] = None):
        ...
        # Core HTTP headers sent in every request
        self.headers = {
    ...
    async def init_checkout(self, session) -> Tuple[bool, str]:
        ...
        if '/cart' in url_lower and '/checkouts' not in url_lower: return False, "Redirected to Cart (Product OOS or Cart Empty)"
        ...
        if not self.sst: return False, f"Failed to get session token. URL: {self.checkout_url}"
        ...
        if self.build_id: self.headers.update({'x-checkout-web-build-id': self.build_id, 'x-checkout-web-deploy-stage': 'production'})
        if self.source_token: self.headers['x-checkout-web-source-id'] = self.source_token
            ...
            'variables': {
                ...
                'merchandise': {
                    'merchandiseLines': [{
                        ...
                        'quantity': {'items': {'value': 1}}, 'expectedTotalPrice': {'value': {'amount': self.subtotal, 'currencyCode': self.currency}},
                ...
                'tip': {'tipLines': []},
                'taxes': {
                    'proposedAllocations': None, 'proposedTotalAmount': {'value': {'amount': '0.00', 'currencyCode': self.currency}},
                ...
                'nonNegotiableTerms': None, 'scriptFingerprint': {'signature': None, 'signatureUuid': None, 'lineItemScriptChanges': [], 'paymentScriptChanges': [], 'shippingScriptChanges': []},
            ...
            try:
                ...
                if payment.get('__typename') == 'FilledPaymentTerms':
                    for method in payment.get('availablePaymentLines', []):
                        ...
                        # Advanced checking: if they don't have a direct credit card parser, they might only use ShopPay/Paypal.
                        if pm.get('__typename') in ['DirectPaymentMethod', 'CustomerCreditCardPaymentMethod', 'PaymentProvider', 'LocalPaymentMethodConfig']:
                ...
                if data:
                    ...
                    if payment.get('__typename') == 'FilledPaymentTerms':
                        for method in payment.get('availablePaymentLines', []):
                            pm = method.get('paymentMethod', {})
                            if pm.get('paymentMethodIdentifier') or pm.get('name'):
                                self.payment_identifier = pm.get('paymentMethodIdentifier', self.payment_identifier)
                                self.gateway = pm.get('extensibilityDisplayName') or pm.get('name', self.gateway)
                                break
...
async def process_card_async(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None) -> Tuple[bool, str, str, str, str, str, float, float]:
    ...
    try:
        # Utilizing curl_cffi AsyncSession with Chrome fingerprinting
