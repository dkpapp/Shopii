    ...
    async def negotiate_delivery(self, session) -> Tuple[bool, str]:
        ...
        try:
            ...
            if data:
                ...
                # Forcefully update the running total to prevent PAYMENTS_UNACCEPTABLE_PAYMENT_AMOUNT
                ...
                # Re-validate the Tax Amount for correct price display
                tax = seller_proposal.get('tax', {})
                if tax.get('__typename') == 'FilledTaxTerms':
                    self.tax_amount = float(tax.get('totalTaxAmount', {}).get('value', {}).get('amount', self.tax_amount))
                    
                # Re-validate the payment identifier to ensure it hasn't mutated
                payment = seller_proposal.get('payment', {})
                if payment.get('__typename') == 'FilledPaymentTerms':
                    for method in payment.get('availablePaymentLines', []):
                        pm = method.get('paymentMethod', {})
