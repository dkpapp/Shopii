...
class ShopifyCheckoutSession:
    ...
    async def negotiate_shipping(self, session) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Running Total
            rt = seller_proposal.get('runningTotal', {})
            if rt:
                self.running_total = rt.get('value', {}).get('amount', '0.00')
    ...
    async def negotiate_delivery(self, session) -> Tuple[bool, str]:
        ...
        try:
            ...
            if data:
                ...
                rt_data = seller_proposal.get('runningTotal', {})
                if rt_data:
                    self.running_total = rt_data.get('value', {}).get('amount', self.running_total)
        except Exception:
            pass # Non-fatal if we fail to parse the updated snapshot
