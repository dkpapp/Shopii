...
CURRENCY_TO_COUNTRY = {
    ...
    "EUR": "FR", "INR": "IN", "AED": "AE", "HKD": "HK",
    "CHF": "CH", "NZD": "NZ", "SGD": "SG", "JPY": "JP",
    "CNY": "CN"
}

# Optimized Global Address Book for minimizing State/Provincial Sales tax & matching local stores
ADDRESS_BOOK = {
    ...
    "IN": {"address1": "Bandra Kurla Complex, G Block", "address2": "", "city": "Mumbai", "zoneCode": "MH", "postalCode": "400051", "countryCode": "IN", "phone": "+919820012345"},
    "AE": {"address1": "Sheikh Zayed Rd, Trade Centre 1", "address2": "", "city": "Dubai", "zoneCode": "DU", "postalCode": "00000", "countryCode": "AE", "phone": "+97143321111"},
    "HK": {"address1": "Nathan 88", "address2": "", "city": "Kowloon", "zoneCode": "KL", "postalCode": "999077", "countryCode": "HK", "phone": "+85255555555"},
    "CN": {"address1": "8 Zhongguancun Street", "address2": "", "city": "Beijing", "zoneCode": "BJ", "postalCode": "100080", "countryCode": "CN", "phone": "+861062512345"},
    "CH": {"address1": "Gotthardstrasse 17", "address2": "", "city": "Schweiz", "zoneCode": "SZ", "postalCode": "6430", "countryCode": "CH", "phone": "+41445512345"},
...
def resolve_address(url: str, currency: Optional[str] = None) -> Dict[str, str]:
    """Resolves the domestic shipping profile to dodge expensive international shipping."""
    if currency:
        cc = CURRENCY_TO_COUNTRY.get(currency.upper())
...
class ShopifyCheckoutSession:
    def __init__(self, site_url: str, cc_info: Dict[str, str], variant_id: Optional[str] = None, proxy_str: Optional[str] = None):
        ...
        # Initial address resolution (updates dynamically later when currency is known)
        ...
        self.address2 = self.address_info.get("address2", "")
    ...
    def _sync_address_to_currency(self) -> None:
        """Dynamically shifts the shipping address to match the store's domestic country to minimize costs."""
        ...
        for endpoint in endpoints:
            try:
                async with session.get(endpoint, proxy=self.proxy, timeout=20) as resp:
                    ...
                    for product in products:
                        for variant in product.get('variants', []):
                            ...
                            try:
                                ...
                                # Finding absolute lowest price & prioritizing digital (no shipping required)
                    ...
                    # Break early if we found acceptable variants
                    if min_digital_variant or min_physical_variant:
                        break
            except Exception as e:
                last_error = str(e)
                
        # Prefer digital items under $15. Otherwise, take the cheapest physical product.
    ...
    async def init_checkout(self, session) -> Tuple[bool, str]:
        ...
        async with session.post(self.site_url + '/checkout/', allow_redirects=True, headers=checkout_headers, proxy=self.proxy) as resp:
            ...
            # Robust token extraction handling multiple checkout paths
            ...
            # Sync address to match local domestic currency avoiding international tax/ship
    ...
    async def negotiate_shipping(self, session) -> Tuple[bool, str]:
        ...
        for attempt in range(3):
            ...
            try:
                ...
                # Handling Shopify's PendingTerms delayed polling calculations
                if delivery.get('__typename') == 'PendingTerms' or tax.get('__typename') == 'PendingTerms':
                    await asyncio.sleep(1.5)
                    continue
                
                # Fetch absolute minimum shipping method available
            ...
            'input': {
                ...
                'delivery': {
                    'deliveryLines': [{
                        ...
                        'targetMerchandiseLines': {'lines': [{'stableId': self.stableId}]},
                        'deliveryMethodTypes': ['SHIPPING'], 'expectedTotalPrice': {'value': {'amount': self.shipping_amount, 'currencyCode': self.currency}},
                        'destinationChanged': False
                ...
                'taxes': {
                    'proposedAllocations': None, 'proposedTotalAmount': {'value': {'amount': self.tax_amount, 'currencyCode': self.currency}},
